...
void context_switch(Process *next, char *arg){
  ...
  if(strcmp(arg, "QUANTUM")==0){
    ...
    update_dados(janela_SCHEDULER, 1, "PID %d finished execution", running_process->pid);
...
int exec_Instruction(Process *process, Opcode opcode, int arg){
  ...
  switch(opcode){
    case READ...WRITE: {
      ...
      update_dados(janela_process, 0, "PID %d request %s", process->pid,opcode_to_string(opcode));
      return IOException;
      break;
    }
    case PRINT:
