...
void sem_P(Process *process, char sem_name) {
    ...
    if (sem->value <= 0) {
        ...
        update_dados(janela_process, 3, "PID %d has semaphore '%c'", process->pid, sem_name, sem->value, sem->blocked_count, MAX_BLOCKED_PROCESSES);
...
void sem_V(Process *process, char sem_name) {
    ...
    if (sem->blocked_count > 0) {
        ...
        update_dados(janela_process, 2, "PID %d leaves semaphore %c",  unblocked_pid, sem_name, sem->blocked_count);
